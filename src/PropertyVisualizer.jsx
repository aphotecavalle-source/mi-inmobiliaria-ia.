    ...
    <div className="min-h-screen bg-[#F8F8F6] font-sans text-stone-800 pb-20">
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex w-full md:w-auto bg-stone-100 p-1 rounded-xl border border-stone-200">
            <button 
              ...
              className={`flex-1 md:flex-none px-4 md:px-6 py-2.5 rounded-lg flex justify-center items-center text-[11px] md:text-xs transition-all ${
      ...
      <main className="max-w-6xl mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-4 gap-6 md:gap-12">
        <aside className="order-2 lg:order-1 lg:col-span-1 space-y-3">
          <h3 className="text-lg md:text-2xl font-extrabold tracking-tight text-stone-900 mb-6">
            Explorar Espacios
          </h3>
            ...
            {propertyData.rooms.map((room) => (
              <button
                ...
                className={`w-full text-left p-4 rounded-xl transition-all ${
                  activeRoom.id === room.id ? 'bg-white border border-stone-200 shadow-lg' : 'bg-transparent'
                }`}
              >
                <div className="flex justify-between items-center">
                  <span className={`text-sm ${activeRoom.id === room.id ? 'font-bold' : 'font-medium text-stone-500'}`}
          ...
          <AnimatePresence mode="wait">
            {viewMode === 'images' ? (
              <motion.div 
                ...
                className="bg-white p-2 rounded-[2.5rem] shadow-2xl border border-stone-100"
              >
                <div className="relative rounded-[2rem] overflow-hidden h-[350px] md:h-[550px]">
                  <div className="absolute top-6 left-6 z-10 bg-black/40 backdrop-blur-md text-white px-5 py-2 rounded-full text-[11px] font-black tracking-widest uppercase">HOY</div>
                  <div style={{ backgroundColor: brandConfig.brandColor }} className="absolute top-6 right-6 z-10 text-white px-5 py-2 rounded-full text-[11px] font-black tracking-widest shadow-lg uppercase flex items-center gap-2">
              ...
              <motion.div 
                ...
                initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                className="bg-stone-950 rounded-[2.5rem] shadow-2xl overflow-hidden aspect-video border-[10px] border-white"
              >
      ...
      <footer className="text-center p-12 text-stone-300 text-[10px] font-bold tracking-widest uppercase">
