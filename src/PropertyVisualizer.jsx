          ...
          <div className="grid grid-cols-1 gap-1">
            {propertyData.rooms.map((room) => (
              <button 
                ...
                className={`w-full text-left p-4 border-b border-stone-100 transition-all ${activeRoom.id === room.id ? 'bg-white shadow-md' : 'text-stone-400 hover:text-stone-900'}`}
                style={activeRoom.id === room.id ? { color: mainColor, borderLeft: `3px solid ${mainColor}`, fontWeight: '600' } : {}}
              >
          ...
          <AnimatePresence mode="wait">
            {viewMode === 'images' ? (
              <motion.div key={activeRoom.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white p-2 shadow-2xl border border-stone-100 relative">
                <button onClick={() => setShowZoomModal(true)} className="absolute top-16 left-6 z-20 bg-white/95 p-2.5 rounded-full shadow-lg transition-all hover:scale-110" style={{ color: mainColor }}>
                  <Maximize2 size={18} />
                </button>
                
                <p className="text-[11px] text-center mb-4 font-bold tracking-[0.35em] uppercase" style={{ color: mainColor }}>
