    ...
    <div className="min-h-screen bg-[#e2ede7] text-[#2a2a2a] pb-32 relative" style={{ fontFamily: 'var(--fuente-sans)' }}>
      ...
      <AnimatePresence>
        {showZoomModal && (
          ...
          >
            <button className="absolute top-6 right-6 text-white/70 hover:text-white"><X size={32} strokeWidth={1} /></button>
            <motion.img 
      ...
      {/* BOTÓN WHATSAPP */}
      <motion.button
        whileHover={{ scale: 1.05 }} onClick={handleWhatsApp}
      ...
      <header className="bg-white/90 backdrop-blur-md sticky top-0 z-50 border-b border-stone-100 p-6">
        <div className="max-w-5xl mx-auto flex flex-col gap-6">
          <div className="flex flex-wrap items-center gap-4 text-[10px] font-bold tracking-[0.2em] uppercase border-b border-stone-50 pb-4">
            <div className="flex items-center" style={{ color: '#c5b097' }}><User size={12} className="mr-2" /> {propertyData.agentName}</div>
            <div className="flex items-center text-stone-400 ml-auto gap-4">
              <Info size={12} /> <button onClick={() => setShowFicha(true)}>DETALLES</button>
            </div>
          </div>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 text-left">
            <div onClick={alRegresar} className="cursor-pointer group">
              <h1 className="editorial-text text-3xl md:text-4xl mb-1 group-hover:opacity-60 transition-opacity">{propertyData.title}</h1>
              <p className="flex items-center text-stone-400 text-[9px] font-bold tracking-widest uppercase"><MapPin size={10} className="mr-2" /> {propertyData.location}</p>
            </div>
            <div className="flex bg-stone-100 p-1 border border-stone-200">
              <button onClick={() => setViewMode('images')} className={`px-5 py-2 flex items-center text-[9px] tracking-widest ${viewMode === 'images' ? 'bg-white shadow-sm font-bold' : 'text-stone-400'}`} style={{ color: viewMode === 'images' ? '#c5b097' : '' }}><ImageIcon size={14} className="mr-2" /> FOTOS</button>
              <button onClick={() => setViewMode('video')} className={`px-5 py-2 flex items-center text-[9px] tracking-widest ${viewMode === 'video' ? 'bg-white shadow-sm font-bold' : 'text-stone-400'}`} style={{ color: viewMode === 'video' ? '#c5b097' : '' }}><Play size={14} className="mr-2" /> VIDEO</button>
            </div>
          </div>
        </div>
      </header>
        ...
        <aside className="order-2 lg:order-1 lg:col-span-1">
          ...
          <div className="grid grid-cols-1 gap-2">
            {propertyData.rooms.map((room) => (
              <button key={room.id} onClick={() => { setActiveRoom(room); setViewMode('images'); }} className={`w-full text-left p-4 transition-all border-b border-stone-200 ${activeRoom.id === room.id ? 'bg-white shadow-sm font-bold' : 'bg-transparent text-stone-400'}`}>
                <div className="flex justify-between items-center"><span className="text-[9px] tracking-widest uppercase" style={{ color: activeRoom.id === room.id ? '#c5b097' : '' }}>{room.name}</span><ChevronRight size={12} /></div>
              </button>
            ))}
          </div>
        </aside>
          ...
          <AnimatePresence mode="wait">
            {viewMode === 'images' ? (
              <motion.div key={activeRoom.id + "-img"} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white p-2 shadow-xl border border-stone-100 relative group">
                <button onClick={() => setShowZoomModal(true)} className="absolute top-16 right-5 z-20 bg-white/80 p-2 rounded-full text-stone-500 hover:text-[#c5b097] shadow-md transition-all"><Maximize2 size={18} /></button>
                <div className="mb-4 text-center"><p className="text-[9px] font-bold tracking-[0.3em] text-stone-400 uppercase italic">Desliza para ver el estado actual</p></div>
                <div className="relative overflow-hidden aspect-video w-full bg-[#f8f8f8]">
                  <ReactCompareSlider position={0} itemOne={<ReactCompareSliderImage src={activeRoom.before} style={{ objectFit: 'contain' }} />} itemTwo={<ReactCompareSliderImage src={activeRoom.after} style={{ objectFit: 'contain' }} />} className="h-full w-full" />
                </div>
              </motion.div>
            ) : (
