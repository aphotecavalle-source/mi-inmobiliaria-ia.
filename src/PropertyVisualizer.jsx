...
const PropertyVisualizer = ({ alRegresar }) => {
  ...
  return (
    <div className="min-h-screen bg-[#e2ede7] text-[#2a2a2a] pb-32 relative" style={{ fontFamily: 'var(--fuente-sans)' }}>
      ...
      {/* BOTÓN WHATSAPP OPTIMIZADO: Más pequeño en móvil */}
      <motion.button
        whileHover={{ scale: 1.05 }}
        onClick={handleWhatsApp}
        style={{ backgroundColor: 'var(--color-arena)', borderRadius: '0px' }}
        className={`fixed z-[100] flex items-center justify-center gap-3 text-white shadow-2xl transition-all 
          ${isMobileLandscape ? 'bottom-4 right-4 p-3' : 'bottom-6 right-6 md:bottom-10 md:right-10 p-4 md:px-8 md:py-5'}`}
      >
      ...
      <header className={`bg-white/90 backdrop-blur-md sticky top-0 z-50 border-b border-stone-100 transition-all ${isMobileLandscape ? 'p-2' : 'p-6'}`}>
        <div className="max-w-5xl mx-auto flex flex-col gap-6">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
              <h1 className={`editorial-text mb-1 ${isMobileLandscape ? 'text-xl' : 'text-3xl md:text-4xl'}`}>
                {propertyData.title}
              </h1>
              {!isMobileLandscape && (
      ...
      {/* CONTENIDO PRINCIPAL */}
      <main className={`max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8 transition-all ${isMobileLandscape ? 'p-2 mt-0' : 'p-6 mt-4'}`}>
